...
class uploadThread (threading.Thread):
    def __init__(self):
        ...
        self.running = True
    def run(self):
        print time.ctime(time.time()) + ': Starting upload Thread'
        connection2 = sqlite3.connect('/home/pi/CarTrackingData.db') 
        cursor2 = connection2.cursor()
        while True:
          print "test upload"
          cursor2.execute("SELECT * FROM gps ORDER BY timestamp DESC LIMIT 1;")
          test2 = cursor2.fetchone()
          print test2
...
if __name__ == '__main__':
  ...
  try:
    ...
    upload.start()
    params=(str(datetime.now()), "RASPBERRY PI STARTED", "", "", "", "", "", "")
    cursor.execute("INSERT INTO gps VALUES (null, ?,?,?,?,?,?,?,?)", params)
    while True:
      if gpsd.fix.latitude != 0.0 and math.isnan(gpsd.fix.latitude)==False:
        gpsLock = True
        ...
        gpsLock = False
        params=(str(datetime.now()), "NO GPS FIX", "", "", "", "", "", "")
        cursor.execute("INSERT INTO gps VALUES (null, ?,?,?,?,?,?,?,?)", params)
        connection.commit() 
